...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByName('btn_sidebar');window.addEventListener('resize',function(event){console.log("resize",event)
...
for(let item of all_monitoring_divs){if(item.id==div_to_display_id){item.classList.remove('d-none')}else{item.classList.add('d-none')}}
open_monitoring()}
function determine_session(element){console.log("div to display",element)
let all_sessions_div=document.getElementById('all_sessions_div');all_sessions_div.style.display="none";let matches_in_session_div=document.getElementById('matches_in_session');matches_in_session_div.classList.remove('d-none')
