...
if(create_sch_match_form){const stu_errov_div=document.getElementById("student_errors");const reader_errov_div=document.getElementById("reader_errors");const day_times_errov_div=document.getElementById("day_times_errors");const reader_choices=document.getElementsByName("reader_choice");const student_choices=document.getElementsByName("student_choice");const meeting_choices=document.getElementsByName("meeting_choice");for(let option of reader_choices){option.addEventListener('click',(event)=>{let the_option=document.getElementById(event.target.id);let the_value=the_option.value;let coor_option=document.getElementById('id_reader_'+the_value);coor_option.click();});}
...
if(checked_reader){let checked_reader_id=checked_reader.id;let choice_to_check_id=checked_reader_id.split('_')[2]+'_choice_reader';let choice_to_check=document.getElementById(choice_to_check_id);choice_to_check.click();}
if(checked_day_times){let checked_day_times_id=checked_day_times.id;let choice_to_check_id=checked_day_times_id.split('_')[3]+'_choice_day_times';let choice_to_check=document.getElementById(choice_to_check_id);choice_to_check.click();}}});