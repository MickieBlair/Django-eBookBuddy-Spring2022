...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByClassName('sidebar_btn');console.log('all_side_bar_buttons',all_side_bar_buttons)
if(staff_outer_div_presence){window.addEventListener('load',(event)=>{for(let item of all_monitoring_divs){if(item==todays_sessions_div){item.classList.remove('d-none')}else{item.classList.add('d-none')}}});window.addEventListener('click',(event)=>{for(let item of all_side_bar_buttons){console.log("id of target",e.target.id)
open_monitoring()}});}
...
function open_monitoring(){console.log("open_monitoring",element)
