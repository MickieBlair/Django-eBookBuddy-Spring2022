...
let vol_role_options=document.getElementsByName("roles");for(let option of vol_role_options){option.addEventListener('change',(event)=>{let the_option=document.getElementById(event.target.id);let the_value=the_option.value;let the_label=the_option.parentElement.textContent.trim();if(the_label=="Volunteer"){event.preventDefault();}else{console.log("Not Volunteer");}
