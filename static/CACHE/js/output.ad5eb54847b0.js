...
showTime();};console.log("Loading Session Scripts");const staff_outer_div_presence=document.getElementById('outer_div_staff');const jitsi_div=document.getElementById('jitsi_meeting_div');let last_width_monitoring;let last_width_video;const todays_session_button=document.getElementById('todays_session_btn');const todays_sessions_div=document.getElementById('todays_session_btn_show')
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByName('btn_sidebar');const all_sessions_div=document.getElementById('all_sessions_div');const matches_in_session_div=document.getElementById('matches_in_session');window.addEventListener('resize',function(event){console.log("resize",event)
let current_width=staff_outer_div_presence.offsetWidth;console.log("current_width",current_width);console.log("last_width_monitoring",last_width_monitoring);console.log("last_width_video",last_width_video);let new_size_for_video=current_width-80-last_width_monitoring;last_width_video=new_size_for_video;jitsi_div.style.width=new_size_for_video+'px';},true);function determine_div_to_display(target){console.log("div to display",target)
let div_to_display_id=target.id+"_show"
for(let item of all_monitoring_divs){if(item.id==div_to_display_id){item.classList.remove('d-none')
if(target.id=="todays_session_btn"){all_sessions_div.style.display="block";matches_in_session_div.classList.add('d-none')
matches_in_session_div.style.display="none";}}else{item.classList.add('d-none')}}
open_monitoring()}
function determine_session(element){console.log("div to display",element)
all_sessions_div.style.display="none";matches_in_session_div.classList.remove('d-none')
matches_in_session_div.style.display="block";const outer_session_divs=document.querySelectorAll('.outer_session_div');let id_for_outer_div_show="outer_session_div_"+element.getAttribute("value")
console.log("div to id_for_outer_div_show",id_for_outer_div_show)
for(let item of outer_session_divs){console.log("item outer_session_divs",item)
if(item.id==id_for_outer_div_show){item.style.display="block";}else{item.style.display="none";}}}
if(staff_outer_div_presence){window.addEventListener('load',(event)=>{for(let item of all_monitoring_divs){if(item==todays_sessions_div){item.classList.remove('d-none');matches_in_session_div.classList.add('d-none');}else{item.classList.add('d-none')}}});}
function close_monitoring(element){console.log("close_monitoring",element)
let staff_left_panel=document.getElementById('staff_left_panel');staff_left_panel.classList.add('d-none');let drag_handle=document.getElementById('drag_handle');drag_handle.classList.add('d-none');let new_sidebar=document.getElementById('new_sidebar');let staff_sidebar_div_width=new_sidebar.offsetWidth;let staff_outer_div_presence_width=staff_outer_div_presence.offsetWidth;let new_width_of_meeting=staff_outer_div_presence_width-staff_sidebar_div_width
let video_div=document.getElementById('jitsi_meeting_div');video_div.style.width=new_width_of_meeting+"px";}
function open_monitoring(){let monitoring_pane_div=document.getElementById('monitoring_pane');let class_list=monitoring_pane_div.classList
if(class_list.value.search('d-none')!=-1){let new_width_of_monitoring=last_width_monitoring+'px';monitoring_pane_div.style.width=new_width_of_monitoring;monitoring_pane_div.classList.remove('d-none');let staff_drag_bar=document.getElementById('staff_drag');staff_drag_bar.classList.remove('d-none');let staff_sidebar_div=document.getElementById('staff_sidebar');let new_width_of_meeting=last_width_video+'px';let video_div=document.getElementById('staff_meeting_div');video_div.style.width=new_width_of_meeting;}}
...
if(width_of_right_now>245){if(width_of_left_now>0){staff_sidebar.css('width',"80px");staff_left_panel.css('width',str_width_left);staff_right_panel.css('width',str_width_right);}else{isResizing=false;}}else{isResizing=false;}}).on('mouseup touchend',function(e){isResizing=false;});};});};;