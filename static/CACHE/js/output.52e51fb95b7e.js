...
var window_width=$(window).width();var handle_width=handle.width();console.log('handle_width',handle_width);var container_width=container.width()-handle_width;var sidebar_width=staff_sidebar.width();console.log('sidebar_width',sidebar_width);let half_width=(container_width-sidebar_width)/2;last_width_monitoring=half_width
last_width_video=half_width
staff_left_panel.css('width',half_width+"px");staff_right_panel.css('width',half_width+"px");var staff_left_panel_width=staff_left_panel.width();var staff_right_panel_width=staff_right_panel.width();if(container){handle.on('mousedown touchstart',function(e){isResizing=true;lastDownX=e.clientX;if(e.type=='touchstart'){isResizing=true;lastDownX=e.originalEvent.touches[0].pageX;console.log('touchstart isResizing',isResizing);console.log('touchstart lastDownX',lastDownX);}else if(e.type=='mousedown'){isResizing=true;lastDownX=e.clientX;console.log('Mouse down isResizing',isResizing);console.log('mouse down lastDownX',lastDownX);}});$(document).on('mousemove touchmove',function(e){if(!isResizing)
return;if(e.type=='touchmove'){lastDownX=e.changedTouches[0].clientX;}else if(e.type=='mousemove'){lastDownX=e.clientX;}
var container_width=container.width();var sidebar_width=staff_sidebar.width();console.log('sidebar_width',sidebar_width);let width_of_left_now=lastDownX-sidebar_width
...
if(width_of_right_now>=250){if(width_of_left_now>0){staff_sidebar.css('width',"80px");if(width_of_left_now<=250){console.log("less than 250")}else{console.log("over than 250")}
staff_left_panel.css('width',str_width_left);staff_right_panel.css('width',str_width_right);}else{isResizing=false;}}else{isResizing=false;}}).on('mouseup touchend',function(e){isResizing=false;console.log('End isResizing ',isResizing);});};});};;