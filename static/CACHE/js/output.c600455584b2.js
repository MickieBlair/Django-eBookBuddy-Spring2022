...
showTime();};console.log("Loading Session Scripts");const staff_outer_div_presence=document.getElementById('staff_outer_div');const jitsi_div=document.getElementById('staff_meeting_div');let last_width_monitoring;let last_width_video;const todays_session_button=document.getElementById('todays_session_btn');const todays_sessions_div=document.getElementById('todays_session_btn_show')
...
if(staff_outer_div_presence){window.addEventListener('load',(event)=>{matches_in_session_div.style.display="none";for(let item of all_monitoring_divs){if(item==todays_sessions_div){item.classList.remove('d-none')}else{item.classList.add('d-none')}}});}
