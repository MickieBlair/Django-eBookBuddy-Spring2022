...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByClassName('sidebar_btn');function determine_div_to_display(target_id){console.log("div to display",target_id)
let div_to_display_id=target_id+"_show"
...
for(let item of all_side_bar_buttons){item.addEventListener('click',(event)=>{determine_div_to_display(event.target.id)});}});function close_monitoring(element){console.log("close_monitoring",element)
