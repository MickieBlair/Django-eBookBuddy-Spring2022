...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');if(staff_outer_div_presence){window.addEventListener('load',(event)=>{for(let item of all_monitoring_divs){if(item==todays_sessions_div){item.classList.remove('d-none')}else{item.classList.add('d-none')}}};}
