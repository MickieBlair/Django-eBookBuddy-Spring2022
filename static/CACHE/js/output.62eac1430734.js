...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByClassName('sidebar_btn');if(staff_outer_div_presence){window.addEventListener('load',(event)=>{for(let item of all_monitoring_divs){if(item==todays_sessions_div){item.classList.remove('d-none')}else{item.classList.add('d-none')}}});all_side_bar_buttons.addEventListener('click',(event)=>{for(let item of all_side_bar_buttons){console.log("id of target",event.target.id)
let state_of_pane_element=document.getElementById('monitoring_pane');console.log(state_of_pane_element)
open_monitoring()}});}
