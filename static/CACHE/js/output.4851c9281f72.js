...
if(edit_sch_match_form){const stu_errov_div=document.getElementById("student_errors");const reader_errov_div=document.getElementById("reader_errors");const day_times_errov_div=document.getElementById("day_times_errors");const reader_choices=document.getElementsByName("reader_choice");const student_choices=document.getElementsByName("student_choice");const new_meeting_choices=document.getElementsByName("new_meeting_choice");const new_day_times_errov_div=document.getElementById("new_day_times_errors");for(let option of new_meeting_choices){option.addEventListener('change',(event)=>{new_day_times_errov_div.innerHTML="";});}
...
const new_checked_day_times=document.querySelector('input[name="new_day_times"]:checked');if(checked_day_times){let checked_day_times_id=checked_day_times.id;let choice_to_check_id=checked_day_times_id.split('_')[3]+'_choice_day_times';let choice_to_check=document.getElementById(choice_to_check_id);if(choice_to_check){choice_to_check.checked=true;}}
