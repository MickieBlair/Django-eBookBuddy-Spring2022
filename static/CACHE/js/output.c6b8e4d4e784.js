...
const all_monitoring_divs=document.getElementsByClassName('monitoring_type');const all_side_bar_buttons=document.getElementsByName('btn_sidebar');window.addEventListener('resize',function(event){console.log("resize",event)},true);function determine_div_to_display(target){let div_to_display_id=target.id+"_show"
