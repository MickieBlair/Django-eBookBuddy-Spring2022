...
if(edit_sch_match_form){const stu_errov_div=document.getElementById("student_errors");const reader_errov_div=document.getElementById("reader_errors");const day_times_errov_div=document.getElementById("day_times_errors");const reader_choices=document.getElementsByName("reader_choice");const student_choices=document.getElementsByName("student_choice");const meeting_choices=document.getElementsByName("meeting_choice");for(let option of reader_choices){option.addEventListener('change',(event)=>{reader_errov_div.innerHTML="";});}
...
coor_option.click()});}}
window.addEventListener('load',(event)=>{if(edit_sch_match_form){const checked_student=document.querySelector('input[name="student"]:checked');const checked_reader=document.querySelector('input[name="reader"]:checked');const checked_day_times=document.querySelector('input[name="day_times"]:checked');if(checked_student){let checked_student_id=checked_student.id;let choice_to_check_id=checked_student_id.split('_')[2]+'_choice_student';let choice_to_check=document.getElementById(choice_to_check_id);choice_to_check.checked=true;}
