...
var window_width=$(window).width();console.log('window_width',window_width);var container_width=container.width();console.log('container_width',container_width);var sidebar_width=staff_sidebar.width();console.log('sidebar_width',sidebar_width);let half_width=(container_width-sidebar_width)/2;staff_left_panel.css('width',half_width+"px");staff_right_panel.css('width',half_width+"px");var staff_left_panel_width=staff_left_panel.width();console.log('staff_left_panel_width',staff_left_panel_width);var staff_right_panel_width=staff_right_panel.width();console.log('staff_right_panel_width',staff_right_panel_width);if(container){handle.on('mousedown touchstart',function(e){isResizing=true;lastDownX=e.clientX;console.log('e',e);if(e.type=='touchstart'){isResizing=true;lastDownX=e.originalEvent.touches[0].pageX;console.log('touchstart isResizing',isResizing);console.log('touchstart lastDownX',lastDownX);}else if(e.type=='mousedown'){isResizing=true;lastDownX=e.clientX;console.log('Mouse down isResizing',isResizing);console.log('mouse down lastDownX',lastDownX);}});$(document).on('mousemove touchmove',function(e){console.log("Doing something")
if(!isResizing)
return;if(e.type=='touchmove'){lastDownX=e.changedTouches[0].clientX;console.log('touchstart isResizing',isResizing);console.log('touchstart lastDownX',lastDownX);}else if(e.type=='mousemove'){lastDownX=e.clientX;console.log('Mouse down isResizing',isResizing);console.log('mouse down lastDownX',lastDownX);}
console.log("lastDownX",lastDownX)
var container_width=container.width();console.log('container_width',container_width);var sidebar_width=staff_sidebar.width();console.log('sidebar_width',sidebar_width);let width_of_left_now=lastDownX-sidebar_width
console.log('width_of_left_now',width_of_left_now);let width_of_right_now=container_width-width_of_left_now
console.log('width_of_right_now',width_of_right_now);let total_width=width_of_right_now+width_of_left_now+sidebar_width
console.log("total_width",total_width)
let str_width_left=width_of_left_now+"px"
let str_width_right=width_of_right_now+"px"
if(width_of_right_now>245||width_of_left_now<=8){staff_sidebar.css('width',"75px");staff_left_panel.css('width',str_width_left);staff_right_panel.css('width',str_width_right);}else{isResizing=false;}}).on('mouseup touchend',function(e){isResizing=false;console.log('End isResizing ',isResizing);});};});;